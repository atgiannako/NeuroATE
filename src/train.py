...
def train_step(sess, dataset, sequence_number, model, transition_params_trained, parameters):
    ...
    feed_dict = {
      model.input_token_gazetteer: batch['gazetteer_indices'],
...
def prediction_step(sess, dataset, dataset_type, model, transition_params_trained, stats_graph_folder, epoch_number, parameters, dataset_filepaths):
    ...
    for i in tqdm(range(0,len(dataset.token_indices[dataset_type]), parameters['batch_size'])):
        ...
        for unary_score, y, sequence_length, predictions, j in zip(batch_unary_scores, batch['label_indices'], batch['sequence_lengths'], batch_predictions, sequence_number):
            if parameters['use_crf']:
...
def restore_model_parameters_from_pretrained_model(parameters, dataset, sess, model, model_saver):
    ...
    pretraining_parameters = load_parameters(parameters_filepath=os.path.join(pretrained_model_folder, 'model/parameters.ini'), verbose=False)[0]
    ...
    if pretraining_dataset.index_to_token == dataset.index_to_token and pretraining_dataset.index_to_character == dataset.index_to_character:
        ...
        model_saver.restore(sess, parameters['saved_model']) # Works only when the dimensions of tensor variables are matched.
